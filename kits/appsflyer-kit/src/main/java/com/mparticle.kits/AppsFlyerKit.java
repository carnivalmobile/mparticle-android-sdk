...
/**
 * Embedded version of the AppsFlyer SDK v 3.3.0
 */
public class AppsFlyerKit extends AbstractKit implements ActivityLifecycleForwarder, ECommerceForwarder {
    ...
    protected AbstractKit update() {
        AppsFlyerLib.setAppsFlyerKey(properties.get(DEV_KEY));
            ...
            if (event.getProductAction().equals(Product.ADD_TO_CART)
                ...
                if (event.getProducts().size() > 0) {
                    ...
                    for (Product product : productList) {
                        ...
                        AppsFlyerLib.trackEvent(context, eventName, productEventValues);
    ...
    public List<ReportingMessage> logEvent(MPEvent event) throws Exception {
        AppsFlyerLib.trackEvent(context, event.getEventName(), new HashMap<String, Object>(event.getInfo()));
        List<ReportingMessage> messages = new LinkedList<ReportingMessage>();
        messages.add(ReportingMessage.fromEvent(this, event));
        return messages;
    }
    ...
    public List<ReportingMessage> setOptOut(boolean optOutStatus) {
        AppsFlyerLib.setDeviceTrackingDisabled(optOutStatus);
    ...
    void setUserIdentity(String id, MParticle.IdentityType identityType) {
        if (MParticle.IdentityType.CustomerId.equals(identityType)) {
            ...
            AppsFlyerLib.setUserEmails(AppsFlyerProperties.EmailsCryptType.NONE, id);
        }
    }
